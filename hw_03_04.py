...
def get_upcoming_birthdays(users=None):
    ...
    for user in users:  # перебираємо користувачів
        ...
        bdate = dtdt.strptime(
            bdate, "%Y.%m.%d"
        ).date()  # перетворюємо дату народження в об’єкт date
        week_day = bdate.isoweekday()  # Отримуємо день тижня (1-7)
        days_between = (
            bdate - tdate
        ).days  # рахуємо різницю між зараз і днем народження цьогоріч у днях
        if 0 <= days_between < 7:  # якщо день народження протягом 7 днів від сьогодні
            if week_day < 6:  #  якщо пн-пт
                birthdays.append(
                    {"name": user["name"], "birthday": bdate.strftime("%Y.%m.%d")}
                ...
                if (bdate + dt.timedelta(days=1)).weekday() == 0:  # якщо неділя
                    birthdays.append(
                        {
                            "name": user["name"],
                            "birthday": (bdate + dt.timedelta(days=1)).strftime(
                                "%Y.%m.%d"
                            ),
                        }
                    )
                    ...
                    birthdays.append(
                        {
                            "name": user["name"],
                            "birthday": (bdate + dt.timedelta(days=2)).strftime(
                                "%Y.%m.%d"
                            ),
                        }
                    )
